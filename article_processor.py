...
以下のAI関連記事について、読者目線で「この技術が私たちの生活にどんな変化をもたらすか」を具体例を交えて書いてください。

【文体の要件】
- 完全に「です・ます調」で統一する
- 親しみやすく、自然な会話口調で書く
- 「〜ですね」「〜かもしれません」「〜でしょう」などの柔らかい表現を使う
- 読者に語りかけるような文体（「皆さん」「私たち」など）

【改行ルール（重要）】
- 2-3文ごとに改行する（適度な段落分け）
- 改行は「\n」を使用する
- 文章は完結させて、途中で切れないようにする
- 読みやすい自然な文章の流れを重視する

【内容の要件】
以下の視点から具体的で独創的な影響を分析：
- 日常の小さな変化：「コーヒーを淹れている間に企画書が完成」「電車の中で外国語の記事がスラスラ読める」
- 仕事の劇的変化：「会議の議事録が自動で要点整理される」「クリエイティブな作業に集中できる時間が増える」
- 意外な活用方法：「おばあちゃんがAIと俳句を作る時代」「ペットの体調管理もAIがサポート」
- 社会への波及効果：「地方の小さな企業でも世界レベルの分析が可能」

一般的な表現（「効率的になる」「便利になる」「豊かになる」など）は避けて、読者が「へえ、そうなんだ！」と思える具体的で新鮮な視点で書いてください。

記事タイトル: {title}
記事要約: {summary}

読者への影響分析（250字程度、です・ます調で1文ごと改行、具体的で独創的な視点で）:
"""
    ...
    def generate_user_value_comment(self, article: NewsArticle, summary: str) -> str:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "あなたは技術の社会的影響を独創的な視点で分析する専門家です。一般的でありきたりな表現を避け、読者が驚くような具体的で新鮮な視点を提供してください。"},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=400,
                temperature=0.8
...
2. 導入文: 今日のAI動向の概要（親しみやすい文体で100字程度、1文ごと改行）
3. 各記事の紹介: 記事ごとに要約と感想を含めて紹介（1文ごと改行）
4. まとめ: 全体を通してのユーザーへのメッセージ（親しみやすい文体で100字程度、1文ごと改行）
